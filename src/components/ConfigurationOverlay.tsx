...
export function ConfigurationOverlay() {
  ...
  useEffect(() => {
    // Check if configuration was previously skipped
    const wasSkipped = localStorage.getItem('skipConfigCheck') === 'true';
    if (wasSkipped) {
      setConfig({ gemini: true, supabaseUrl: true, supabaseAnonKey: true, axonEncryptionKey: true });
      return;
    }
  ...
  const handleSkip = () => {
    localStorage.setItem('skipConfigCheck', 'true');
    setConfig({ gemini: true, supabaseUrl: true, supabaseAnonKey: true, axonEncryptionKey: true });
  };
        ...
        <div className="max-w-xl w-full bg-slate-900 border border-slate-800 rounded-2xl p-8 shadow-2xl relative overflow-hidden">
          ...
          <div className="relative z-10 flex flex-col items-center text-center">
            ...
            <button 
              onClick={checkConfig}
              disabled={loading}
              className="flex items-center gap-2 px-6 py-3 bg-blue-600 hover:bg-blue-500 disabled:opacity-50 text-white rounded-lg font-medium transition-colors w-full justify-center mb-3"
            >
